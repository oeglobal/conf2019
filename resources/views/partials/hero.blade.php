<div class="bg-grey-lighter">
  <div class="hero flex items-center justify-center flex-col lg:max-w-5xl mx-auto">
    <div class="container text-center flex-col lg:text-left">
      <div class="branding justify-center lg:justify-start flex">@svg('hero-branding', 'h-40 w-auto pl-2')</div>
      <div class="flex">
        <div class="tagline">Connecting the global <br/>open education community</div>
      </div>
      <div class="name">20-22 November, 2020</div>
    </div>
    <div class="text-white hero-text text-center flex flex-col justify-center items-center">
      <div class="flex items-center justify-center">
        <a href="{!! home_url('/why-attend/') !!}" class="btn">Why attend OE Global?</a>
      </div>
    </div>
  </div>
</div>
